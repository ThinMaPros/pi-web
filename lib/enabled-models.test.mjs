...
test("renaming a provider rewrites its entries and keeps the rest", () => {
  const result = renameProviderEntries(
    input(["my-gateway/alpha:high", "my-gateway/*", "anthropic/*", "my-gateway-old/x"]),
...
test("renaming several providers at once is one rewrite", () => {
  ...
  assert.deepEqual(result.patterns, ["x/one", "y/two", "c/three"]);
});

test("renaming is a no-op without patterns or without a real rename", () => {
  assert.equal(renameProviderEntries(input(undefined), [{ from: "a", to: "b" }]).changed, false);
  const before = input(["anthropic/*"]);
  assert.equal(renameProviderEntries(before, []).changed, false);
  assert.equal(renameProviderEntries(before, [{ from: "a", to: "a" }]).changed, false);
  assert.deepEqual(renameProviderEntries(before, [{ from: "x", to: "y" }]).patterns, before.patterns);
});
